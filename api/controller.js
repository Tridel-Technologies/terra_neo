...
const createFolderAndFile = async (req, res) => {
  ...
  try {
    ...
    for (const fname of file_name) {
      ...
      const fileResult = await pool.query(fileInsertQuery, [fname, folderId, units.waterLevel, units.currentSpeed, units.currentDirection, units.battery, units.depth, "dd"]);
