    ...
    try {
      ...
      if (filter.length > 0) {
        const targetDateTime = new Date(
          `${filter[0].date.substring(0, 10)}T${filter[0].time}`
        );
        ...
        const beforeSixHours = this.main_table.filter((item) => {
          const itemDateTime = new Date(
            `${item.date.substring(0, 10)}T${item.time}`
          );
          return (
            itemDateTime.getTime() < targetDateTime.getTime() - sixHoursInMs
          );
        });
      ...
      this.toast.error(
        'This file has no high_water_level data please edit in process page',
        'Error'
      );
    }
  }
