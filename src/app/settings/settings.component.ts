...
onBlankAreaRightClick(event: MouseEvent) {
  ...
  if (this.folderName) {
    ...
    this.http.post('http://localhost:3000/api/create_folder', {folder_name:this.folderName}).subscribe(
      (response:any)=>{
...
removeFileFromOriginalFolder(fileToRemove: fileData) {
  for (let folder of this.files_list) {
    const index = folder.files.findIndex(file => file.file_id === fileToRemove.file_id);
    if (index !== -1) {
...
pasteFile(targetFolder: any) {
  ...
  if (this.fileToMove && targetFolder) {
    ...
    console.log("start")
    const fromFolderIndex = this.files_list.findIndex(f => f.folder_id === this.fileToMove.fromFolder);
    ...
    const toFolderIndex = this.files_list.findIndex(f => f.folder_id === targetFolder.folder_id);
    if (toFolderIndex !== -1) {
...
changeFiles(folder:any){
  ...
  console.log(data);
  this.http.post('http://localhost:3000/api/change_folder', data).subscribe(
    (response:any)=>{
      console.log(response);
...
onContainerRightClick(event: MouseEvent) {
  ...
  if ((event.target as HTMLElement).classList.contains('fileCContainer')) {
    ...
    if (folderName) {
      ...
      //   files: []
      // });
      this.http.post('http://localhost:3000/api/create_folder', {folder_name:folderName}).subscribe(
      (response:any)=>{
        console.log(response)
        this.init();
      }
...
constructor(private unitService: UnitService, private http:HttpClient, private toastr:ToastrService) {
