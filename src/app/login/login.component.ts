    ...
    if (this.userName_signup && this.Email_id_signup && this.password_signup) {
      ...
      this.loginservice.signup_user(newdata).subscribe({
        next: (res) => {
          ...
          this.toastr.error('Data successfully sent to server!', 'Success');
          this.signup_form=false;
          this.login_form=true;
        },
        error: (err) => {
          if (err.status === 201) {
            ...
            this.toastr.error('Something went wrong while sending data to the server', 'Validation Error');
          ...
          if (res.valid) {
            this.emailVerified = true;
              ...
              if (res.reason === 'username') {
                  this.toastr.error('❌ Username does not exists', 'Validation Error');
               } else if (res.reason === 'email') {
                  this.toastr.error('❌ Email_id does not exists', 'Validation Error');
               } else {
